...
function Extractor( options ) {
	...
	this.extendLastNDays = (options && options.extendLastNDays)? options.extendLastNDays : 0;
}
		...
		while (x < i) {
			...
			var item = items[x];
				...
				if(context.useExtendedParsing )	{
					context.extendParse( item, fileItems, fileItemsParsed );

				// if within extendLastNDays - use extended version which parses twitter.com page	  	
				}else if(context.extendLastNDays && context.extendLastNDays > 0){
			 		var pub = new Date(item.created_at),
			            now = new Date(),
			            timespan = 1000 * 60 * 60 * 24 * context.extendLastNDays;

			        if(pub.getTime() > (now.getTime() - timespan)) {
			            console.log('***** extending item - extendLastNDays *****')
			            context.extendParse( item, fileItems, fileItemsParsed );
			        }else{
...
Extractor.prototype.extendParse = function ( item, fileItems, fileItemsParsed ){
	var context = this;
