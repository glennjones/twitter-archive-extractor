...
Extractor.prototype.merge = function ( extData, data, callback ){
	...
	this.mergeInReplyTo( extData, data );
	this.mergeComment( extData, data );
	this.mergeLike( extData, data );
...
/*		extData.items[0].type[1] = 'h-as-video';*/
		callback(null, data, 'video');

	}else{
		callback(null, data, 'note');
	}
...
Extractor.prototype.mergeComment = function ( extData, data ){
	...
	if(extData.items[0].properties.comment ){
		...
		while (x < i) {
			...
			while (y < z) {
				if( comments[y].properties.url[0] === extComment[x].properties.url[0]){
					comments[y] = extComments[x]
					found = true;
					console.log('found like by', extComment[x].properties.url[0])
				}
				y++;
			}
			if(!found){
				comments.push( extComment[x] )
				console.log('not found adding', extComment[x].properties.url[0])
			}
		    x++;
		}
		...
		data.items[0].properties.comments = comments;
		...
		while (x < i) {
			...
			while (y < z) {
				if( likes[y].properties.author[0].properties.url[0] === extLikes[x].properties.author[0].properties.url[0]){
					likes[y] = extLikes[x]
					found = true;
					console.log('found like by', extLikes[x].properties.author[0].properties.url[0])
				}
				y++;
			}
			if(!found){
				likes.push( extLikes[x] )
				console.log('not found adding', extLikes[x].properties.author[0].properties.url[0])
			}
		    x++;
		}
...
Extractor.prototype.mergeInReplyTo = function ( extData, data ){
	...
	if(extData.items[0].properties['in-reply-to'] ){
		...
		while (x < i) {
			...
			if(!found){
				...
				console.log('not found adding', extReplies[x].properties.url[0])
			}
  		...
  		if(entry.items[0].type && entry.items[0].type.indexOf('h-entry') > -1){
			...
			// has sync property with twitter status url pattern
  			...
  			if(syndicationLink){
				...
				extendedNote.add({'url':url, 'tweet': updatedEntry}, function(err, extData, data){
					
						context.merge( extData, data, function( err, out){
							console.log( 'merge done', JSON.stringify(out) );
						});
  				...
  				callback('has no twitter status syndication link', entry)
  			}
  		}else{
	  		callback('this object does not have type property h-entry', entry)
	  	}
  	}else{
  		callback('is not a object or does not have items collection', entry)
  	}
