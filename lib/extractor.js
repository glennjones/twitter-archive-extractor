				...
				fs.readFile(filePath, 'utf-8', function(err, contents) { 
					...
					while (x < i) {
						...
						if( context.isEntryInDateRange(tweets[x]) ){
							...
							console.log('excuted tweet created on: ', tweet.created_at)
						}
						x++;
					}
...
Extractor.prototype.isfileNameInDateRange = function ( name ){
	...
	var startNum = this.fileStartYear.toString() + this.fileStartMonth.toString()
	console.log(startNum, fileNum)
...
// does a tweet have username
Extractor.prototype.hasUsername = function ( tweet ){
...
// does a tweet have urls
Extractor.prototype.hasURLs = function ( tweet ){
