...
Extractor.prototype.extract = function( options, callback ){
	...
	console.log('extract from', this.startDate);
...
/*	this.on('tag', function(data){ 
	...
	console.log(this.fileStartYear,this.fileStartMonth)
...
Extractor.prototype.isComplete = function(){
	//console.log('fileCount', 'completedFiles', 'itemCount', 'completedItems + erroredItems')
	console.log(this.fileCount, this.completedFiles, this.itemCount, this.completedItems + this.erroredItems, '(', this.completedItems, this.erroredItems, ')')
...
Extractor.prototype.readProfileData = function( photoURL ){
	...
	fs.readFile(filePath, 'utf-8', function(err, contents) {
		if(contents) {
			...
			profile.parse( userDetails, photoURL, emitFn );
		}else{
			context.emit('error', err);
			emitFn( err, null )
		}
	...
	fs.readdir(folderPath, function(err, files) {
		...
		while (i--) {
		    ...
		    if( context.isfileNameInDateRange(file) ){
				...
				context.emitAndCount(null, {}, 'excluded-file', 0, 0, 0 );
			}
...
Extractor.prototype.readTweetFile = function( filePath, parsedPeople ){
	...
	fs.readFile(filePath, 'utf-8', function(err, contents) { 
		...
		// reverse loop into right date order
		items.reverse();
		...
		if(items.length > 0){
			...
			while (x < i) {
				...
				var item = items[x];
					...
					// add the photo url to user profile
					// only run once per an extraction
					if(item 
						&& item.user 
						&& item.user.profile_image_url_https
						&& context.foundPhoto === false ){
						console.log(item.user.profile_image_url_https)
						context.foundPhoto = true;
						context.readProfileData( item.user.profile_image_url_https );	
					}
					...
					if(context.useExtendedParsing )	{
						context.parseExtendedTweet(item , function(err, extData, tweet){
							context.parseTweet(tweet, function(err, data, name){
								if(err){
									fileItemsParsed +=1;
									context.emitAndCount(err, data, name, items.length, fileItemsParsed, 1 );
								}else{
			...
			// if file fell in range, but none of its items did
			context.emitAndCount(null, {}, 'all-items-excluded', 0, 0, 0 );
		}
...
Extractor.prototype.emitAndCount = function (err, data, name, fileItems, fileItemsParsed, completedItems){
	console.log(name, fileItems, fileItemsParsed)
...
Extractor.prototype.parseExtendedTweet = function ( tweet, callback ){
	// http://tools.transmat.io/twitter/glennjones/status/501368889981554690/
	var url = 'http://tools.transmat.io/twitter/' + tweet.user.screen_name + '/status/' + tweet.id_str + '/';
  		...
  		if(entry.items[0].type && entry.items[0].type.indexOf('h-entry') > -1){
			...
			// has sync property with twitter status url pattern
  			...
  			if(syndicationLink){
				...
				// get extended data
  				var extendedNote = new ExtendedNote();
  				var url = 'http://tools.transmat.io/twitter/' + syndicationLink.replace('https://twitter.com/','') + '/';
...
Extractor.prototype.isfileNameInDateRange = function ( name ){
	var fileNum = name.replace('.js','').replace('_',''),
		startNum = this.fileStartYear.toString() + this.pad(this.fileStartMonth);

	//console.log(fileNum, '>=', startNum, parseInt(fileNum, 10), parseInt(startNum, 10) )
...
Extractor.prototype.pad = function(number) {
	var r = String(number);
	if ( r.length === 1 ) {
	    r = '0' + r;
	}
