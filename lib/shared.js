...
module.exports = {
	...
	parseCommon: function( name, tweet ){
		...
		if(tweet.created_at){
			...
			// this is important as it stops item from been added to update que in transmat system
			prop.modified = [this.formatDate(tweet.created_at)];
		}
	...
	parseHTML: function ( text, tweet ){
		...
		while (i--) {
			...
			if (word.indexOf('pic.twitter.com/') === 0) {
				words[i] = '<a class="auto-link u-photo" href="https://' + word + '">' + word + '</a>';
			}
		}
		return words.join(' ');
	},
